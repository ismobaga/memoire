...
num_req = 0
USERS = list(range(5, 50, 5))
for fit_func in functions:
    results[fit_func.__name__] = [0 for v in range(len(USERS))]
...
uid = f"g{GENERATIONS}-p{POPULATION_SIZE}-u{USERS[0]}_{USERS[-1]}"


fig = plt.figure(figsize=(10, 6))
plt.xlabel("Users")
plt.ylabel("% taches")
plt.title(f"Pourcentage de tache execute par N_USERS avec une moyen de {num_req}")
...
plt.savefig(f"users-{uid}.png", format='png')
tikzplotlib_fix_ncols(fig)
import tikzplotlib
tikzplotlib.save(f"users-{uid}.pgf")
